...
* @param [in] 	-pin_number: 	Specifies the GPIO pin to be configured
*						 	 	must be set based on @ref GPIO_PINs_Define.
* @param [in] 	-pin_mode:		Specifies mode for the selected pin
*								can be set based on @ref GPIO_MODE_Define.
* @param [in] 	-pin_speed:		Specifies speed for the selected pin
*								can be set based on @ref GPIO_SPEED_Define.
* @retval 		-none
* Note			-Stm32F103x8 MCU has GPIO A,B,C,D,E Modules
*/


void GPIO_Init (GPIO_TypeDef *GPIOx, uint16_t pin_number , uint16_t pin_mode , uint16_t pin_speed )
{
	GPIO_PIN_Config PinConfig ;
	PinConfig.pin_number = pin_number ;
	PinConfig.pin_mode = pin_mode ;
	PinConfig.pin_speed = pin_speed ;
	GPIO_Init_ConFig (GPIOx, &PinConfig ) ;
}
...
uint8_t GPIO_LockPin (GPIO_TypeDef *GPIOx, uint16_t PinNumber )
{
	...
	volatile uint32_t temp = 0x10000 ;
	temp |= PinNumber ;
	...
	// Read 1 (this read is optional but confirms that the lock is active)
	if((uint32_t)(GPIOx->LCKR & 0x10000))
	{
