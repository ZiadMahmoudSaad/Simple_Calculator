...
void UPDATE_EXTI(EXTI__PIN_Config_t* EXTI_Config)
{
	...
	GPIO_Init_ConFig(EXTI_Config->EXTI_PIN.GPIO_PORT, &PINcfg);
...
void EXTI_GPIO_Init_ConFig(EXTI__PIN_Config_t *EXTI_Config)
{
	UPDATE_EXTI(EXTI_Config) ;
}

void EXTI__GPIO_Update__ConFig(EXTI__PIN_Config_t* EXTI_Config)
{
	UPDATE_EXTI(EXTI_Config) ;
}
//=========================================================
...
* @param [in] 	-EXTI_PIN: 			Specifies the EXTI GPIO mapping.
*									This parameter must be set based on @red EXTI_Define
* @param [in] 	-Trigger_Case:		Specifies Rising or falling or Both Trigger.
* 									This parameter must be set based on @ref EXTI_Trigger_Define
* @param [in] 	-IRQ_EN:			Enable Or Disable the EXTI_IQR.
* 									his will enable the IRQ MASK in EXTI and also on the NVIC Interrupt controller.
*									This parameter must be set based on @red EXTI_IRQ_Define
* @param [in] 	-P_IRQ_CallBack:	Set the Function() which will be called once the IRQ Happen
* @retval 		-none
* Note			-none
*/
...
void EXTI_GPIO_Init(EXTI_GPIO_Mapping_t EXTI_PIN , uint8_t Trigger_Case, uint8_t IRQ_EN , void (* P_IRQ_CallBack)(void))
{
	EXTI__PIN_Config_t EXTI_Config ;
	EXTI_Config.EXTI_PIN = EXTI_PIN ;
	EXTI_Config.Trigger_Case = Trigger_Case ;
	EXTI_Config.IRQ_EN = IRQ_EN ;
	EXTI_Config.P_IRQ_CallBack = P_IRQ_CallBack ;
	EXTI_GPIO_Init_ConFig(&EXTI_Config);
}
...
void EXTI__GPIO_Update(EXTI_GPIO_Mapping_t EXTI_PIN , uint8_t Trigger_Case, uint8_t IRQ_EN , void (* P_IRQ_CallBack)(void))
{
	EXTI__PIN_Config_t EXTI_Config ;
	EXTI_Config.EXTI_PIN = EXTI_PIN ;
	EXTI_Config.Trigger_Case = Trigger_Case ;
